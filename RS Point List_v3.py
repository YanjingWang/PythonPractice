...
df = pd.read_excel('rs point list 2023 updated 08-29-22 12-16-22.xlsx',header=0) # rs point list 2023 updated 08-29-22 # rs point list 2023 updated 08-29-22 11-21-22 #rs point list 2023 updated 08-29-22 11-21-22
df = pd.DataFrame(df)
df =df.applymap(str)
df = df[["DBN RPT","LastName","FirstName","RoleName","EmailAddress","UserID"]]
df.rename(columns = {'DBN RPT':'DBN','UserID':'UserName'}, inplace = True)  # 2404 eows
...
##########################################################################################################################
##########################################################################################################################
df = df.drop(['FirstName', 'LastName','EmailAddress'], axis=1)  # which is first???
